...
//     const margin = { top: 0, right: 20, bottom: 0, left: 30 };
...
export class TimelineComponent implements OnChanges {
  ...
  createTimeline() {
    ...
    // Extend the end date a bit to avoid cut-off circles
    const paddedEnd = new Date(this.endDate);
    paddedEnd.setMonth(paddedEnd.getMonth() + 2);

    const xScale = d3
      .scaleTime()
      .domain([this.startDate, paddedEnd])
      .range([0, width]);

    const xAxis = d3.axisBottom(xScale)
      .ticks(d3.timeYear.every(3)) // every 2 years
      ...
      svg.append('circle')
        ...
        .on('mouseover', (event) => {
          tooltip
            .style('visibility', 'visible')
            .html(
              `<strong>Date:</strong> ${d3.timeFormat('%b, %Y')(date)}<br><br>${combinedTooltipText}`
            );
        })
        .on('mousemove', (event) => {
          tooltip
